% Education section

\section{\textcolor{airforceblue}{EDUCATION}}
  \employmentSubheadingListStart
    \employmentSubheading
    {\makebox[0pt][l]{Rotman School of Management, University of Toronto {\mdseries - Toronto, ON}}}{}
    {MBA Candidate}{2027}
      \begin{itemize}[leftmargin=1.5em]
        \item Entrance Scholarship of \$25,000
        \item First Year Representative, Marketing \& Sales Association
        \item Member of Business Technology Association, Health Management Association and Women in Management Association
      \end{itemize}\vspace{-4pt}
    \employmentSubheading
    {\makebox[0pt][l]{St Clair College {\mdseries - Windsor, ON}}}{}
    {Diagnostic Medical Sonography Advanced Diploma}{2021}
      \begin{itemize}[leftmargin=1.5em]
        \item Graduated with Academic Distinction
        \item Student Advisor \& Program Representative
        % \item Ultrasound Student Advisor \& Program Representative: Collaborated with faculty and student services to
% develop engaging workshops increasing student participation by 20\%.
      \end{itemize}\vspace{-4pt}
    \employmentSubheading
    {\makebox[0pt][l]{University of Waterloo {\mdseries - Waterloo, ON}}}{}
    {Bachelor of Science, Honors Biomedical Sciences, Psychology Minor}{2017}
      \begin{itemize}[leftmargin=1.5em]
        \item Entrance Scholarship of \$5,000
      \end{itemize}
  \employmentSubheadingListEnd
  \vspace{-12pt}
